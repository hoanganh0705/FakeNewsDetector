...
Những đặc điểm nàysâu của tiếng Việt đòi hỏi bước tiền xử lý đặc thù -- đặc biệt là \textit{phân đoạn từ} -- trước khi được dùng trong các mô hình học máy và học sâu.
...
RNN là một mô hình học sâu được thiết kế để xử lý dữ liệu tuần tự bằng cách duy trì thông tin từ các trạng thái trước đó~\cite{elman1990rnn}. Điều này giúp RNN có khả năng ghi nhớ và sử dụng các mối liên hệ giữa các phần tử trong chuỗi dữ liệu, tạo ra đầu ra có ngữ cảnh và ý nghĩa hơn. 
...
Cho đến nay, đã có nhiều kỹ thuật trích xuất đặc trưng văn bản được các nhà nghiên cứu đề xuất và áp dụng hiệu quả, chẳng hạn như \textit{Bag-of-Words}, \textit{TF-IDF}~\cite{sparkjones1972idf} hay các biểu diễn dựa trên \textit{word embedding}~\cite{mikolov2013word2vec}. Bên cạnh đó, các mô hình học sâu, đặc biệt là các mạng nơ-ron hồi tiếp và các mô hình ngôn ngữ tiền huấn luyện như \textit{Transformer}~\cite{vaswani2017attention}, cũng chứng minh được tính vượt trội thông qua những kết quả đầy hứa hẹn trên nhiều bộ dữ liệu khác nhau. 
