...
\begin{tabular}{r l}
  \textbf{Sinh viên thực hiện:} & Nguyễn Hoàng Anh \\[0.3em]
...
Tin giả trên các nền tảng số tiếng Việt đang đặt ra thách thức cấp bách cho các nhà nghiên cứu NLP, do đặc điểm hình thái học của tiếng Việt -- ngôn ngữ đơn lập, không biến hình -- vốn đòi hỏi những kỹ thuật tiền xử lý và biểu diễn đặc trưng riêng biệt. Trong khi nhiều nghiên cứu quốc tế đã đạt kết quả đáng chú ý trên dữ liệu tiếng Anh, hiệu quả của các phương pháp tương tự trên tiếng Việt chưa được khảo sát đầy đủ.

Nghiên cứu này đề xuất và so sánh bốn phương pháp phân loại tin tức Việt Nam: Hồi quy Logistic (LR) và Máy vectơ hỗ trợ (SVM) sử dụng đặc trưng tần suất từ -- tần suất tài liệu nghịch đảo (Term Frequency -- Inverse Document Frequency, TF-IDF), Mạng LSTM hai chiều (BiLSTM) với cơ chế chú ý, và mô hình ngôn ngữ PhoBERT tinh chỉnh theo tác vụ. Thực nghiệm được thực hiện trên bộ dữ liệu gồm \nClean{} mẫu sau khi làm sạch, được chia theo tỉ lệ \splitRatio{} (huấn luyện/kiểm định/kiểm tra).

Kết quả cho thấy PhoBERT đạt hiệu năng tốt nhất (F1 = \phobertFone\%, ROC-AUC = \phobertAUC), vượt trội rõ rệt so với LR (\lrFone\%), SVM (\svmFone\%) và BiLSTM (\bilstmFone\%). Đáng chú ý, SVM lại vượt BiLSTM bất chấp kiến trúc đơn giản hơn nhiều, cho thấy chất lượng biểu diễn đặc trưng có thể bù đắp cho độ phức tạp của mô hình khi dữ liệu có giới hạn. Kiểm định McNemar với hiệu chỉnh Holm-Bonferroni xác nhận sự vượt trội của PhoBERT có ý nghĩa thống kê ($p < 0.001$). Nghiên cứu ablation cho thấy tiền huấn luyện trên ngữ liệu tiếng Việt quy mô lớn là nhân tố quyết định; phân đoạn từ và kích thước từ điển chỉ đóng vai trò thứ yếu đối với các mô hình học máy truyền thống.
...
Mạng xã hội và các nền tảng tin tức trực tuyến tại Việt Nam đã rút ngắn tối thiểu khoảng cách giữa sự kiện và công chúng, nhưng cũng đồng thời tạo ra môi trường thuận lợi cho tin sai lệch phát tán rộng rãi trước khi được kiểm chứng. Theo thống kê, Việt Nam nằm trong nhóm các quốc gia có tỉ lệ tiếp xúc với tin giả cao nhất tại Đông Nam Á. Việc kiểm chứng thủ công không thể theo kịp quy mô sản xuất và lưu truyền nội dung hiện nay; đó là lý do các hệ thống phát hiện tin giả tự động bằng học máy và học sâu trở nên cấp thiết.
...
Nghiên cứu tập trung vào phân loại nhị phân văn bản tiếng Việt (Thật/Giả) dựa trên tiêu đề và nội dung bài viết. Dữ liệu được thu thập từ các bài đăng công khai trên mạng xã hội (Facebook) và bào điện tử trong giai đoạn COVID-19, bao gồm ngôn ngữ tự nhiên, không được kiểm soát
...
Chương này thiết lập nền tảng lý thuyết cho bốn mô hình được khảo sát trong nghiên cứu. Phần đầu làm rõ các khái niệm biểu diễn văn bản và đặc thù ngôn ngữ tiếng Việt; phần tiếp theo phân tích nguyên lý hoạt động của hai thuật toán học máy truyền thống -- LR và SVM. Phần cuối đi sâu vào các kiến trúc học sâu: từ LSTM tuần tự, BiLSTM hai chiều, đến Transformer song song, qua đó lý giải vì sao PhoBERT -- nhờ cơ chế self-attention nắm bắt ngữ cảnh toàn cục -- đạt hiệu năng vượt trội trong bài toán phát hiện tin giả tiếng Việt.

\section{Một số khái niệm cơ bản}

\subsection{Văn bản và biểu diễn văn bản}
Mô hình học máy không xử lý trực tiếp chuỗi ký tự mà yêu cầu văn bản phải được chuyển sang dạng số trong một không gian đặc trưng xác định. Hai trường phái biểu diễn chính đối lập nhau về bản chất: Bag-of-Words và TF-IDF ánh xạ mỗi tài liệu thành một vectơ thưa (\textit{sparse vector}) trong không gian từ vựng, hoàn toàn bỏ qua thứ tự từ; ngược lại, các phương pháp embedding chiếu từng từ vào một không gian dày (\textit{dense}) có số chiều thấp, nơi quan hệ ngữ nghĩa được mã hóa ngầm qua cấu trúc hình học của các vectơ. Sự lựa chọn giữa hai trường phái này gắn chặt với kiến trúc mô hình: LR và SVM vận hành hiệu quả trên vectơ TF-IDF thưa nhờ khả năng tối ưu trong không gian chiều cao, còn BiLSTM và PhoBERT đòi hỏi biểu diễn embedding liên tục để khai thác ngữ cảnh tuần tự hoặc toàn cục.

\subsection{Đặc trưng văn bản và tiền xử lý}
Tiền xử lý biến đổi văn bản thô thành đầu vào đồng nhất cho mô hình thông qua chuẩn hóa chữ viết, loại bỏ ký tự nhiễu, tách từ và lọc từ dừng. Đối với tiếng Việt, \textit{phân đoạn từ} (\textit{word segmentation}) đóng vai trò then chốt hơn hẳn so với các ngôn ngữ biến hình: khoảng trắng trong tiếng Việt phân tách âm tiết chứ không phân tách từ, nên nhiều đơn vị từ vựng bao gồm hai hoặc nhiều âm tiết liền kề (``kiến\_trúc'', ``tiền\_huấn\_luyện''). Bỏ qua bước này khiến từ vựng phân mảnh nghiêm trọng, đồng thời làm giảm chất lượng vectơ TF-IDF và gây bất tương thích giữa dữ liệu đầu vào với từ điển tiền huấn luyện của PhoBERT.
...
Phát hiện tin giả về bản chất là bài toán phân loại nhị phân: mỗi văn bản được gán nhãn “thật” hoặc “giả”. Mô hình cần học một hàm ánh xạ từ không gian đặc trưng của văn bản sang tập nhãn, sao cho ranh giới quyết định phân tách tốt nhất hai lớp trên dữ liệu chưa quan sát.
...
Những đặc điểm này đòi hỏi bước tiền xử lý đặc thù -- đặc biệt là \textit{phân đoạn từ} -- trước khi áp dụng các mô hình học máy.

\subsection{Phân đoạn từ (Word Segmentation)}

Phân đoạn từ là bước xác định ranh giới các từ trong câu tiếng Việt. Hầu hết các mô hình xử lý ngôn ngữ tự nhiên (Natural Language Processing -- NLP), đặc biệt là PhoBERT, được huấn luyện trên dữ liệu đã được phân đoạn.
...
Trong nghiên cứu này, nhóm sử dụng \textbf{VnCoreNLP RDRSegmenter}~\cite{nguyen2018vncorenlp} thông qua thư viện \texttt{py\_vncorenlp}. Đây là bộ phân đoạn cùng với bộ tokenizer mà \phobert được tiền huấn luyện, đảm bảo sự nhất quán về từ vựng.

\subsection{Biểu diễn văn bản số}

Các mô hình học máy cần chuyển đổi văn bản thô sang dạng số. Nghiên cứu này sử dụng ba phương pháp:

\begin{enumerate}[leftmargin=2.5em]
  \item \textbf{TF-IDF:} Vectơ đặc trưng dựa trên tần suất từ, phù hợp cho LR và SVM.
  ...
  \item \textbf{Mã hóa cặp byte (Byte-Pair Encoding -- BPE):} Tách từ thành các đơn vị con, dùng cho PhoBERT.
\end{enumerate}
...
Với sự phát triển của khoa học công nghệ, các ứng dụng tự động ngày càng trở
nên phổ biến hơn trong đời sống chúng ta, một phần là nhờ vào các thuật toán học
máy. Học máy (Machine Learning -- ML) là một nhánh của lĩnh vực trí tuệ nhân tạo
(Artificial Intelligence -- AI), chuyên nghiên cứu và phát minh các thuật toán cho phép
máy tính có khả năng tự vận hành một công việc nhất định bằng các bộ dữ liệu đã
có mà không cần can thiệp nhiều từ lập trình viên. Các thuật toán máy học thường
được chia thành 2 loại chính, đó là:

Học có giám sát (Supervised Learning) gồm các thuật toán giúp máy tính xây
dựng một hàm dựa trên dữ liệu có sẵn và đã được gán nhãn để có thể xác định nhãn
của một dữ liệu mới.

Học không giám sát (Unsupervised Learning) gồm các thuật toán cho phép máy
tính học từ các dữ liệu có sẵn nhưng chưa được gán nhãn. Từ đó, phân các dữ liệu
này thành các cụm theo yêu cầu.
...
Mạng neuron nhân tạo là một tập hợp gồm các neuron nhân tạo được sắp xếp
theo từng lớp ẩn. Mỗi lớp ẩn (hidden layer) bao gồm một hoặc nhiều
neuron nhân tạo. Các neuron này liên kết từng đôi một với các neuron ở lớp kế tiếp
và sẽ không có sự liên kết giữa các neuron trong cùng một lớp. Số lớp của một mạng
neuron nhân tạo được tính bằng tổng số lớp ẩn và lớp đầu vào, lớp đầu ra không
được tính.
...
RNN là một mô hình học sâu được thiết kế để xử lý dữ liệu tuần tự bằng cách duy trì thông tin từ các trạng thái trước đó. Điều này giúp RNN có khả năng ghi nhớ và sử dụng các mối liên hệ giữa các phần tử trong chuỗi dữ liệu, tạo ra đầu ra có ngữ cảnh và ý nghĩa hơn. 
...
Cơ chế chú ý (Attention Mechanism)~\cite{bahdanau2015attention} được đề xuất nhằm khắc phục hạn chế của các mô hình xử lý chuỗi khi phải nén toàn bộ thông tin của một câu vào một vector duy nhất. Thay vì xem mọi phần tử trong chuỗi có mức độ quan trọng như nhau, cơ chế chú ý cho phép mô hình tự động học trọng số cho từng bước thời gian, từ đó tập trung nhiều hơn vào những thành phần mang thông tin quan trọng đối với nhiệm vụ dự đoán.
...
Trong đó, $\alpha_t$ phản ánh mức độ đóng góp của từng từ hoặc từng bước thời gian vào biểu diễn cuối cùng của câu. Những từ quan trọng sẽ nhận được trọng số lớn hơn, trong khi các từ ít liên quan sẽ có trọng số nhỏ hơn. Nhờ đó, mô hình có khả năng tập trung vào các thành phần mang tính quyết định trong văn bản thay vì xử lý toàn bộ chuỗi một cách đồng đều.
...
Transformer là một kiến trúc học sâu dựa trên cơ chế tự chú ý (self-attention)~\cite{vaswani2017attention}, được đề xuất nhằm khắc phục những hạn chế của các mô hình tuần tự như RNN và LSTM trong việc xử lý phụ thuộc dài hạn. Không giống như các mô hình xử lý dữ liệu theo từng bước thời gian, Transformer cho phép xử lý toàn bộ chuỗi đầu vào đồng thời, từ đó tăng khả năng song song hóa và cải thiện hiệu quả huấn luyện.

Kiến trúc Transformer ban đầu bao gồm hai thành phần chính là encoder và decoder. Tuy nhiên, trong các mô hình ngôn ngữ tiền huấn luyện như BERT~\cite{devlin2019bert} và PhoBERT~\cite{phobert2020}, chỉ phần encoder được sử dụng để xây dựng biểu diễn ngữ cảnh cho văn bản. Do đó, trong phạm vi đề tài này, trọng tâm được đặt vào kiến trúc Transformer encoder.
...
Thành phần thứ hai là mạng truyền thẳng theo từng vị trí (Positionwise Feed-Forward Network -- FFN), thực hiện các phép biến đổi phi tuyến trên từng vector đầu ra của lớp attention. Sau đó, một bước Add \& Norm tiếp theo được áp dụng. Các khối encoder này được xếp chồng nhiều lần nhằm tăng khả năng biểu diễn của mô hình.

Nhờ khả năng học biểu diễn ngữ cảnh toàn cục thông qua self-attention và xử lý song song hiệu quả, Transformer đã trở thành nền tảng cho nhiều mô hình ngôn ngữ hiện đại. PhoBERT~\cite{phobert2020} được xây dựng dựa trên kiến trúc RoBERTa~\cite{liu2019roberta}, sử dụng Transformer encoder nhiều tầng và được tiền huấn luyện trên lượng lớn dữ liệu tiếng Việt.

Trong bài toán phát hiện tin giả, vector tương ứng với token đặc biệt \texttt{[CLS]} được sử dụng làm biểu diễn toàn cục của văn bản và được đưa qua một lớp phân loại để dự đoán nhãn ``thật'' hoặc ``giả''. Việc tận dụng biểu diễn ngữ cảnh sâu từ PhoBERT giúp mô hình hiểu rõ hơn nội dung và sắc thái của văn bản, từ đó nâng cao hiệu quả phân loại.
...
Bảng~\ref{tab:dataset} trình bày số lượng mẫu, phân phối nhãn và độ dài trung bình của văn bản trong từng tập dữ liệu. Có thể thấy rằng tin thật chiếm \pctReal\% và tin giả chiếm \pctFake\% ở cả ba tập (huấn luyện, kiểm định và kiểm tra), cho thấy chiến lược chia dữ liệu theo phương pháp phân tầng (\textit{stratified split}) đã bảo toàn phân phối nhãn ban đầu.

Độ dài trung bình của văn bản trong tập kiểm định và kiểm tra (\avgLenVal\ từ đối với tập kiểm định và \avgLenTest\ từ đối với tập kiểm tra) cao hơn nhẹ so với tập huấn luyện (\avgLenTrain\ từ), phản ánh sự đa dạng về cấu trúc văn bản trong toàn bộ dữ liệu. Mặc dù tồn tại sự mất cân bằng lớp ở mức vừa phải, tỷ lệ này không quá nghiêm trọng và được xử lý thông qua cơ chế trọng số lớp (\textit{class weighting}) trong quá trình huấn luyện nhằm giảm thiểu sai lệch dự đoán.

\subsection{Kiểm tra rò rỉ dữ liệu}
Để đảm bảo tính khách quan và độ tin cậy của quá trình đánh giá, nhóm tiến hành kiểm tra hiện tượng rò rỉ dữ liệu (\textit{data leakage}) giữa các tập huấn luyện, kiểm định và kiểm tra sau khi thực hiện chia dữ liệu.

Cụ thể, toàn bộ nội dung văn bản trong từng tập (Train, Validation, Test) được chuyển đổi thành các tập hợp (\textit{set}) dựa trên giá trị của trường \texttt{text}. Sau đó, nhóm thực hiện phép giao giữa từng cặp tập dữ liệu nhằm phát hiện các bản ghi trùng lặp:
...
Việc kiểm tra rò rỉ dữ liệu đặc biệt quan trọng trong các bài toán phân loại văn bản, bởi sự xuất hiện của các mẫu trùng lặp giữa tập huấn luyện và tập kiểm tra có thể dẫn đến đánh giá thiên lệch (\textit{biased evaluation}). Khi đó, mô hình có thể đạt hiệu năng cao một cách giả tạo do đã ``ghi nhớ'' nội dung từ tập huấn luyện, thay vì thực sự học được các đặc trưng có khả năng tổng quát hóa. Do đó, việc đảm bảo tính độc lập giữa các tập dữ liệu là điều kiện tiên quyết để đánh giá chính xác năng lực của mô hình.

\section{Kết luận chương}

Chương này đã trình bày toàn bộ quy trình chuẩn bị dữ liệu cho bài toán 
phát hiện tin giả tiếng Việt, bao gồm phân đoạn từ, làm sạch và chia tập 
theo chiến lược phân tầng. Dữ liệu được phân đoạn bằng RDRSegmenter nhằm 
đảm bảo tính tương thích với mô hình PhoBERT, sau đó được loại bỏ các 
văn bản trùng lặp, rỗng hoặc quá ngắn nhằm giảm nhiễu và nâng cao chất lượng đầu vào.

Tập dữ liệu sau làm sạch được chia theo tỷ lệ 70\%–15\%–15\% cho các tập 
huấn luyện, kiểm định và kiểm tra với \texttt{random\_state = 42}. 
Đồng thời, hệ thống tiến hành kiểm tra nghiêm ngặt rò rỉ dữ liệu giữa các tập 
để đảm bảo không tồn tại giao nhau về nội dung văn bản.

Quy trình này đảm bảo tính nhất quán, khách quan và khả năng tái lập 
(\textit{reproducibility}), tạo nền tảng vững chắc cho quá trình huấn luyện 
và đánh giá mô hình ở chương tiếp theo.
...
Cho đến nay, đã có nhiều kỹ thuật trích xuất đặc trưng văn bản được các nhà nghiên cứu đề xuất và áp dụng hiệu quả, chẳng hạn như \textit{Bag-of-Words}, \textit{TF-IDF} hay các biểu diễn dựa trên \textit{word embedding}. Bên cạnh đó, các mô hình học sâu, đặc biệt là các mạng nơ-ron hồi tiếp và các mô hình ngôn ngữ tiền huấn luyện như \textit{Transformer}, cũng chứng minh được tính vượt trội thông qua những kết quả đầy hứa hẹn trên nhiều bộ dữ liệu khác nhau. 

Vì thế, để lựa chọn phương pháp phù hợp và hiệu quả nhất cho bài toán phát hiện tin giả tiếng Việt, nhóm quyết định tiến hành so sánh hiệu năng giữa các phương pháp trích chọn đặc trưng thủ công kết hợp với các mô hình phân lớp truyền thống và các mô hình học sâu hiện đại. Đối với các phương pháp trích chọn đặc trưng thủ công, nhóm sẽ triển khai các thuật toán phân lớp như \textit{Logistic Regression}, \textit{SVM} và các mô hình học sâu như \textit{BiLSTM} và \textit{PhoBERT} nhằm đánh giá và phân tích hiệu năng một cách toàn diện.
...
\begin{enumerate}[leftmargin=3.3em]
  \item \textbf{Tiền xử lý:} Văn bản được làm sạch nhằm loại bỏ các ký tự không cần thiết, chuẩn hóa dữ liệu và thực hiện phân đoạn từ tiếng Việt. Sau đó, tập dữ liệu được chia thành các tập huấn luyện, kiểm định và kiểm tra theo tỷ lệ phù hợp nhằm đảm bảo tính khách quan trong quá trình đánh giá mô hình.

  \item \textbf{Trích xuất đặc trưng:} Tùy theo từng mô hình, nhóm áp dụng các phương pháp biểu diễn văn bản khác nhau. Cụ thể, \textit{TF-IDF} được sử dụng cho các mô hình \textit{Logistic Regression} và \textit{SVM}; \textit{Word Embedding} (FastText) được sử dụng cho mô hình \textit{BiLSTM}; và phương pháp mã hóa \textit{BPE} (\textit{Byte-Pair Encoding}) được sử dụng trong mô hình \textit{PhoBERT} nhằm khai thác thông tin ngữ cảnh ở mức độ sâu hơn.

  \item \textbf{Phân loại:} Bốn mô hình bao gồm \textit{Logistic Regression}, \textit{SVM}, \textit{BiLSTM} và \textit{PhoBERT} được huấn luyện độc lập trên cùng một tập dữ liệu và được đánh giá thông qua các chỉ số như \textit{Accuracy}, \textit{F1-score} và \textit{ROC-AUC}. Dựa trên các kết quả thu được, nhóm tiến hành so sánh và phân tích hiệu năng của từng phương pháp.
\end{enumerate}
...
Trong các bài toán phân loại văn bản, TF-IDF được xem là một phương pháp
biểu diễn đặc trưng cơ bản nhưng hiệu quả, đặc biệt khi kết hợp với các
mô hình phân lớp tuyến tính như Hồi quy Logistic (Logistic Regression) và Máy vectơ hỗ trợ (Support Vector Machine). Phương pháp này biểu diễn văn bản dưới dạng các vector thưa
(\textit{sparse vectors}), trong đó mỗi chiều phản ánh mức độ quan trọng
của một từ trong văn bản so với toàn bộ tập dữ liệu.

Mặc dù các phương pháp biểu diễn hiện đại như nhúng từ (word embedding) hay các mô
hình ngôn ngữ tiền huấn luyện có khả năng khai thác ngữ nghĩa sâu hơn,
TF-IDF vẫn được sử dụng rộng rãi như một baseline mạnh trong nhiều nghiên
cứu phân loại văn bản. Đặc biệt đối với các mô hình tuyến tính, biểu diễn
TF-IDF giúp khai thác hiệu quả các tín hiệu từ vựng (\textit{lexical cues})
có tính phân biệt cao trong văn bản.

Trong bài toán phát hiện tin giả, các cụm từ giật tít, các mẫu ngôn ngữ
gây chú ý hoặc các từ khóa đặc trưng thường đóng vai trò quan trọng trong
việc phân biệt giữa tin thật và tin giả. Các tín hiệu này có thể được nắm
bắt hiệu quả thông qua các đặc trưng n-gram của TF-IDF. Vì vậy, TF-IDF
kết hợp với Logistic Regression và SVM được sử dụng như một baseline
mạnh nhằm so sánh với các phương pháp học sâu trong nghiên cứu này.
...
\begin{itemize}[leftmargin=2em]
  ...
  Số lượng đặc trưng được giới hạn ở \tfidfMaxFeatures{} từ/ngữ phổ biến
  nhất, giúp giảm nhiễu và kiểm soát độ phức tạp của mô hình.

  \item \textbf{N-gram:}
  Sử dụng kết hợp đơn từ (unigram) và bigram (n-gram = (1, 2)) nhằm khai
  thác cả thông tin ngữ nghĩa đơn lẻ và các cụm từ liên tiếp, từ đó tăng
  khả năng nhận diện các mẫu ngữ cảnh đặc trưng của tin giả.

  \item \textbf{Tần số tài liệu:}
  Các từ xuất hiện quá ít (\texttt{min\_df=5}) hoặc quá phổ biến
  (\texttt{max\_df=0.95}) đều bị loại bỏ, đảm bảo chỉ giữ lại các đặc
  trưng có giá trị phân biệt cao.

  \item \textbf{TF sublinear:}
  Áp dụng phép biến đổi $\text{TF} = 1 + \log f_{t,d}$ nhằm giảm ảnh
  hưởng của các từ xuất hiện nhiều lần trong cùng một văn bản, giúp
  mô hình tập trung vào sự hiện diện của từ hơn là tần suất tuyệt đối.
...
(factor=0.5, patience=2) để điều chỉnh tốc độ học và gradient clipping
...
clipping (max\_norm=1.0) và dừng sớm (patience=2).
...
\begin{itemize}[leftmargin=2em]
  ...
  \item Ablation study: tiền huấn luyện trên tiếng Việt là nhân tố quyết định; phân đoạn từ ảnh hưởng ít với TF-IDF nhưng quan trọng với PhoBERT.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item Xây dựng hệ thống phát hiện thời gian thực với PhoBERT được nén nhỏ (knowledge distillation).
