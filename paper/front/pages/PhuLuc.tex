...
\noindent\textbf{Mẫu tin thật (label = 0, ngắn):}
\begin{quote}\small\itshape
Giáo\_dục hoà\_nhập giúp học\_sinh khuyết\_tật có cơ\_hội học\_tập
cùng các bạn bình\_thường.\hfill(id=5857)
\end{quote}

\noindent\textbf{Mẫu tin thật (label = 0, dài):}
\begin{quote}\small\itshape
``Việc học là suốt đời, chúng\_ta có\_thể tạm hoãn; sức\_khoẻ là
quan\_trọng nhất\ldots''\hfill(id=4519, ĐH Nông lâm, về COVID-19)
\end{quote}
...
Mô hình sinh viên là \texttt{StudentBiLSTM} --- một phiên bản BiLSTM rút gọn với \texttt{embedding\_dim = \studentembdim{}}, \texttt{hidden\_dim = \studenthiddim{}}, \texttt{num\_layers = \studentlayers{}}. Trọng số soft target $\alpha = \studentalpha{}$, nhiệt độ phân phối $T = \studenttemp{}$. Optimizer sử dụng AdamW với \texttt{lr}=$10^{-3}$, scheduler CosineAnnealingLR. Huấn luyện tối đa \studentepochs{} epoch với cơ chế dừng sớm (\texttt{patience}=\studentpatience{}). Hàm mất mát KD chuẩn (Hinton et al., 2015) đã được trình bày ở Mục~\ref{sec:extended_analyses}; mô hình sinh viên được huấn luyện trên tập logits giáo viên đã được tính trước và lưu lại nhằm tiết kiệm chi phí tính toán.
...
Khoảng tin cậy được ước lượng bằng phương pháp bootstrap (\texttt{scipy.stats.bootstrap}) với 10{,}000 lần lặp.
