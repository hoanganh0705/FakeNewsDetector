...
\newcommand{\bilstmParamCount}{$\sim$7.2M}
...
Trong nghiên cứu này, nhóm sử dụng \textbf{VnCoreNLP RDRSegmenter} thông qua thư viện \texttt{py\_vncorenlp}. Đây là bộ phân đoạn cùng với bộ tokenizer mà mô hình \phobert được tiền huấn luyện,do đó đảm bảo được sự nhất quán về từ vựng
...
\begin{enumerate}[leftmargin=2.5em]
  ...
  \item \textbf{Nhúng từ (Word Embedding):} Vectơ dày đặc học được, huấn luyện end-to-end, phù hợp cho BiLSTM.
...
RNN là một mô hình học sâu được xây dựng nhằm xử lý dữ liệu tuần tự thông qua việc giữ lại thông tin từ các trạng thái trước đó. Nhờ đó, RNN có khả năng nhớ và sử dụng các mối liên hệ giữa các phần tử trong dữ liệu tuần tự để tạo ra các kết quả có ngữ cảnh và ý nghĩa hơn. 
...
\begin{figure}[H]
  ...
  \caption{Minh họa cấu trúc mạng LSTM hai chiều.}
  \label{fig:BiLSTM}
\end{figure}
...
Cơ chế chú ý (Attention Mechanism) được đưa ra để cải thiện các nhược điểm của các mô hình xử lý chuỗi khi phải nén toàn bộ thông tin của một câu thành một vector. Thay vì mỗi phần tử trong chuỗi đều được tính là có độ quan trọng ngang nhau, cơ chế chú ý cho phép các mô hình tự động học được trọng số cho từng bước trong chuỗi, để tập trung nhiều hơn vào các phần tử quan trọng đối với các tác vụ dự đoán.
...
Transformer là một kiến trúc học sâu dựa trên cơ chế tự chú ý (self-attention), được đề xuất nhằm giải quyết các hạn chế của các mô hình tuần tự như RNN và LSTM khi áp dụng trong xử lý phụ thuộc dài hạn. So với các mô hình xử lý dữ liệu theo từng bước thời gian,Transformer cho phép xử lý toàn bộ chuỗi dữ liệu đầu vào một cách đồng thời, từ đó tăng khả năng song song hóa và hiệu quả huấn luyện.

Kiến trúc ban đầu củaTransformer bao gồm các thành phần chính là encoder và decoder. Tuy nhiên, trong các mô hình ngôn ngữ tiền huấn luyện như BERT và PhoBERT, chỉ sử dụng phần encoder củaTransformer để xây dựng biểu diễn ngữ cảnh cho văn bản. Vì vậy, trong phạm vi đề tài nghiên cứu này, chúng tôi tập trung vào kiến trúc của Transformer encoder.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{encoder.jpg}
  \caption{Minh họa khối encoder của Transformer.}
  \label{fig:encoder}
\end{figure}
...
Dựa trên khả năng học biểu diễn ngữ cảnh toàn cục thông qua self-attention và khả năng xử lý song song hiệu quả, Transformer đã trở thành nền tảng của các mô hình ngôn ngữ hiện đại. PhoBERT được xây dựng dựa trên kiến trúc của RoBERTa, một Transformer encoder nhiều tầng được tiền huấn luyện trên lượng lớn dữ liệu văn bản tiếng Việt.
...
\begin{enumerate}
    ...
    Đầu vào của các mô hình này là đặc trưng TF-IDF.
  ...
  LSTM. Khi kết hợp với cơ chế attention,
...
Các vector embedding có chiều \bilstmEmbDim{} được khởi tạo ngẫu nhiên và huấn luyện end-to-end cùng với toàn bộ mô hình. Nếu sử dụng bộ embedding tiền huấn luyện FastText cc.vi.300.bin, có thể tải xuống từ trang chủ fasttext.cc và đặt vào thư mục data/fasttext/; mã nguồn hỗ trợ tải ma trận pretrained qua hàm load\_fasttext\_matrix() trong src/features/embedding\_features.py. Việc khởi tạo ngẫu nhiên trong thí nghiệm này được áp dụng để đảm bảo khả năng tái lập mà không phụ thuộc vào file pretrained.



\subsection{Tokenization PhoBERT (BPE)}

Quá trình mã hóa đầu vào cho PhoBERT sử dụng thuật toán Byte-Pair Encoding (BPE) với từ điển gồm 64.000 subword, cho phép mô hình xử lý hiệu quả các từ ngoài từ điển và hiện tượng đa hình trong tiếng Việt. Mỗi văn bản được tiền xử lý như sau:
\begin{itemize}[leftmargin=2em]
...
\begin{itemize}[leftmargin=2em]
  ...
  \item Cấu hình tối ưu: $C = \svmBestC$, LinearSVC kết hợp hiệu chuẩn xác suất Platt (calibration)
\end{itemize}
...
\begin{enumerate}[leftmargin=2.5em]
  ...
  \item \textbf{Classifier:} Hai lớp dropout (0.3) xen kẽ với các lớp tuyến tính và ReLU, giúp giảm overfitting và tăng khả năng biểu diễn phi tuyến.
\end{enumerate}

Chiến lược huấn luyện sử dụng AdamW
...
Thực nghiệm được thực hiện trên hệ thống: GPU NVIDIA (CUDA), RAM 32~GB, Python 3.14, PyTorch 2.10, Transformers 4.57, scikit-learn 1.8 (xem requirements.txt để biết phiên bản chính xác).
...
Nghiên cứu sử dụng ba chỉ số đánh giá hiệu chuẩn:
...
Với hiệu chỉnh Holm-Bonferroni cho 6 phép so sánh:
...
\begin{table}[H]
\centering
\caption{Khoảng tin cậy 95\% Bootstrap cho F1-Score (10,000 lần lấy mẫu lại)}
\label{tab:ci}
\begin{tabular}{lcc}
\toprule
