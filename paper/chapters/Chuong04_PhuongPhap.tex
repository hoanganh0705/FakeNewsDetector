...
Phát hiện tin giả tiếng Việt là một bài toán đáng chú ý trong lĩnh vực xử lý ngôn ngữ tự nhiên, nhất là khi khối lượng nội dung được đăng tải trên không gian mạng ngày càng tăng nhanh. Những thông tin sai lệch nếu được lan truyền rộng rãi có thể tác động tiêu cực đến cách người dùng tiếp nhận thông tin, đồng thời kéo theo nhiều hệ quả xã hội không mong muốn. Trước thực tế đó, việc xây dựng và đánh giá các phương pháp có khả năng tự động nhận diện nội dung sai lệch trở thành một nhu cầu thực tiễn rõ ràng.

Hướng tiếp cận phổ biến hiện nay là chuyển văn bản sang dạng biểu diễn phù hợp trước khi đưa vào mô hình phân loại. Các phương pháp như \textit{bag-of-words}, TF-IDF hay \textit{word embedding} mỗi cách mang lại một góc nhìn khác nhau trong việc mã hóa đặc trưng văn bản. Song song với đó, sự phát triển của học sâu đã mở ra các phương pháp mới có khả năng khai thác thông tin ngữ cảnh trực tiếp từ dữ liệu. Trong nhóm này, mạng nơ-ron hồi tiếp và mô hình ngôn ngữ dựa trên Transformer là hai kiến trúc được ứng dụng rộng rãi nhất, đồng thời cho thấy tiềm năng rõ rệt trong các bài toán phân loại văn bản.
...
Đối với bài toán phân loại văn bản --- và cụ thể là phát hiện tin giả --- việc lựa chọn mô hình cần dung hòa ba yếu tố:
\begin{itemize}
...
Dựa trên các tiêu chí này, nghiên cứu so sánh hai nhóm phương pháp:
\begin{enumerate}
...
\begin{enumerate}
    ...
    \item Hiệu quả mà mô hình PhoBERT mang lại khi tận dụng tri thức ngôn ngữ tiếng Việt đã học trước, áp dụng vào bài toán phát hiện tin giả trên dữ liệu thực tế.
\end{enumerate}

\section{Tổng quan kiến trúc hệ thống}

Quy trình thực nghiệm của hệ thống được triển khai theo ba giai đoạn chính, bao gồm chuẩn bị dữ liệu, xây dựng biểu diễn đầu vào và tiến hành huấn luyện, đánh giá:

\begin{enumerate}[leftmargin=3.3em]

    \item \textbf{Tiền xử lý dữ liệu:} văn bản trước tiên được chuẩn hóa và loại bỏ các thành phần không cần thiết, sau đó thực hiện phân đoạn từ tiếng Việt. Dữ liệu đã xử lý được phân thành ba tập Train, Validation và Test theo tỷ lệ được quy định trước. Việc tách riêng các tập dữ liệu giúp xác định rõ dữ liệu phục vụ quá trình học và dữ liệu dùng để kiểm tra khả năng của mô hình.

    \item \textbf{Biểu diễn dữ liệu:} phương pháp tạo đầu vào được lựa chọn dựa trên đặc điểm của từng kiến trúc. Đối với Logistic Regression và SVM, văn bản được chuyển thành vector đặc trưng bằng TF-IDF. BiLSTM sử dụng các vector từ được xây dựng với FastText, còn PhoBERT nhận văn bản sau khi được mã hóa bằng BPE (\textit{byte-pair encoding}). Biểu diễn sau đó được đưa vào mô hình tương ứng để khai thác thông tin phục vụ phân loại.

    \item \textbf{Huấn luyện và đánh giá:} từng mô hình được huấn luyện riêng trên cùng nguồn dữ liệu và theo thiết lập tương ứng, qua đó tạo điều kiện so sánh kết quả giữa các phương pháp. Hiệu năng được xem xét dựa trên \textit{accuracy}, \textit{F1-score} và \textit{ROC-AUC}. Các kết quả thu được được sử dụng để đối chiếu khả năng phân loại và phân tích sự khác biệt giữa bốn mô hình.
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{N-gram:} biểu diễn được xây dựng từ cả đơn từ (Unigram) và các cặp từ liên tiếp (Bigram), tương ứng với N-Gram = (1,2). Nhờ xét thêm các chuỗi từ, mô hình có thể khai thác những cụm biểu đạt mang thông tin phân biệt mà biểu diễn unigram đơn thuần có thể bỏ qua.
...
Mô hình máy vectơ hỗ trợ (SVM) sử dụng LinearSVC với trọng số lớp cân bằng nhằm kiểm soát ảnh hưởng do phân bố nhãn không đồng đều. Quá trình tối ưu siêu tham số được thực hiện qua GridSearchCV với 5-fold cross-validation:
...
Như đã phân tích ở Mục~\ref{sec:model_selection}, BiLSTM và PhoBERT được lần lượt lựa chọn làm đại diện cho nhóm mô hình tuần tự và nhóm mô hình ngôn ngữ tiền huấn luyện. Hai mô hình được trình bày về cấu trúc kiến trúc và các thiết lập huấn luyện tương ứng.
...
Mặc dù tỷ lệ nhãn trong bộ dữ liệu (\pctreal{}\% \textit{thật} / \pctfake{}\% \textit{giả}) không chênh lệch quá lớn, sự mất cân bằng vẫn có thể ảnh hưởng đến khả năng phân loại --- đặc biệt đối với lớp thiểu số (\textit{tin giả}). Để giảm thiểu tác động này, đồ án áp dụng đồng thời hai chiến lược bổ trợ:
...
Ngoài các độ đo hiệu năng chuẩn (Accuracy, F1, ROC-AUC), chương thực nghiệm sẽ thực hiện bốn nhóm phân tích mở rộng nhằm đánh giá mô hình một cách toàn diện hơn. Mục này mô tả trước các phương pháp được sử dụng; kết quả chi tiết được trình bày trong Chương~\ref{chap:experiments}.
...
Thiết kế này đảm bảo so sánh công bằng trên cùng bộ dữ liệu và cùng quy trình đánh giá --- mỗi mô hình được huấn luyện với cùng phân chia tập, cùng siêu tham số đã được điều chỉnh hợp lý, và cùng ba chỉ số chính (Accuracy, F1-macro, ROC-AUC). Chương tiếp theo trình bày kết quả thực nghiệm của toàn bộ khung phương pháp này.
