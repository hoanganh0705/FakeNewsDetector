...
Chương này trình bày toàn bộ cơ sở phương pháp luận cho bài toán phát hiện tin giả bằng tiếng Việt. Nội dung được tổ chức theo trình tự từ việc làm rõ bài toán, lựa chọn mô hình, mô tả kiến trúc hệ thống đến chi tiết kỹ thuật trích xuất đặc trưng và cấu hình huấn luyện của từng mô hình. Phần cuối chương khái quát các phân tích mở rộng, giải thích mô hình, hiệu chỉnh xác suất, phân tích mẫu khó và knowledge distillation.

\section{Phương pháp đề xuất}

Sự phát triển nhanh của môi trường trực tuyến đã làm thay đổi đáng kể cách thông tin được tạo ra, phân phối và tiếp nhận. Bên cạnh những lợi ích về khả năng tiếp cận thông tin, môi trường này cũng tạo điều kiện cho các nội dung thiếu chính xác lan truyền với tốc độ cao. Đối với tiếng Việt, việc tự động nhận diện những nội dung sai lệch vì thế trở thành một hướng nghiên cứu đáng chú ý trong lĩnh vực xử lý ngôn ngữ tự nhiên, với tiềm năng hỗ trợ cả các nghiên cứu học thuật và những ứng dụng thực tế.

Để đưa văn bản vào các mô hình học máy, nội dung ngôn ngữ cần được chuyển đổi thành dạng biểu diễn mà thuật toán có thể khai thác. Các phương pháp truyền thống như Bag-of-Words và TF-IDF biểu diễn văn bản dựa trên sự xuất hiện hoặc tần suất của các từ, trong khi word embedding cung cấp cách biểu diễn giàu thông tin hơn thông qua các vector số. Tuy nhiên, những phương pháp này vẫn gặp hạn chế trong việc mô tả đầy đủ quan hệ ngữ cảnh của từ trong câu. Các mô hình dựa trên kiến trúc Transformer đã mở rộng khả năng biểu diễn này bằng cách khai thác thông tin ngữ cảnh trong toàn bộ chuỗi văn bản, từ đó đạt hiệu quả cao trong nhiều nhiệm vụ phân loại và phân tích ngôn ngữ.
...
Đối với nhiệm vụ phân loại văn bản,  việc đánh giá các phương pháp có thể dựa trên ba tiêu chí chính:
\begin{itemize}
    \item \textit{Khả năng diễn giải} --- mức độ con người có thể hiểu được logic ra quyết định của mô hình.
    \item \textit{Chi phí tính toán} --- chi phí về thời gian và tài nguyên trong quá trình huấn luyện cũng như suy diễn.
    \item \textit{Năng lực biểu diễn ngữ nghĩa} --- khả năng khai thác những sắc thái ngữ nghĩa trong nội dung văn bản.
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item PhoBERT
    \end{itemize}
    BiLSTM tận dụng hai hướng xử lý của mạng LSTM để khai thác thông tin từ cả phía trước lẫn phía sau của chuỗi. Khi được bổ sung cơ chế \textit{attention}, mô hình có thể gán mức độ quan trọng khác nhau cho từng thành phần trong văn bản, từ đó giúp mô hình ưu tiên những thông tin có giá trị đối với quá trình phân loại~\cite{hochreiter1997lstm,bahdanau2015attention}.

    PhoBERT kế thừa kiến trúc RoBERTa và được phát triển riêng cho ngữ liệu tiếng Việt, với quá trình tiền huấn luyện được thực hiện trên dữ liệu tiếng Việt~\cite{vaswani2017attention,liu2019roberta,phobert2020}. Nhờ học biểu diễn theo ngữ cảnh trong giai đoạn tiền huấn luyện, mô hình có khả năng tiếp thu các thông tin ngữ nghĩa ở nhiều cấp độ. Vì vậy, PhoBERT có thể được áp dụng hiệu quả cho những nhiệm vụ xử lý cũng như phân loại văn bản tiếng Việt.
\end{enumerate}
...
Các vector embedding có chiều \bilstmembdim{} được khởi tạo bằng bộ vector FastText đã được huấn luyện trước cho tiếng Việt~\cite{bojanowski2017fasttext} (\texttt{cc.vi.300.bin}). Phương pháp FastText khai thác các n-gram ở cấp độ ký tự để tạo biểu diễn cho từ, nhờ đó vẫn có thể xây dựng vector cho những từ chưa xuất hiện trong từ điển bằng cách kết hợp các vector n-gram tương ứng. Trong quá trình huấn luyện, các vector này được đưa vào lớp embedding và tiếp tục cập nhật thông qua cơ chế lan truyền ngược, giúp biểu diễn từ thích nghi với đặc điểm của nhiệm vụ phát hiện tin giả. Cách tiếp cận này vừa tận dụng được thông tin ngôn ngữ đã học từ tập ngữ liệu lớn (Common Crawl), vừa cho phép embedding thích ứng với đặc trưng của dữ liệu tiếng Việt trong bài toán.
...
Mô hình Logistic Regression được thiết lập với điều chuẩn L2 để giảm nguy cơ quá khớp, đồng thời sử dụng cơ chế (\texttt{class\_weight='balanced'}) nhằm điều chỉnh ảnh hưởng của sự mất cân bằng giữa các lớp. GridSearchCV được áp dụng để lựa chọn các giá trị siêu tham số phù hợp, trong đó hiệu năng của từng cấu hình được đánh giá thông qua phương pháp 5-fold cross-validation. Dựa trên kết quả thu được qua các lần đánh giá, cấu hình mang lại hiệu năng phù hợp nhất được lựa chọn cho mô hình:
...
Ngoài các độ đo hiệu năng chuẩn (Accuracy, F1, ROC-AUC), chương thực nghiệm sẽ thực hiện bốn nhóm phân tích mở rộng nhằm đánh giá mô hình một cách toàn diện hơn.
